  ...
  const transferWithIndividualAmounts = useCallback(
    async (
      ...
      try {
        ...
        const signedTransactions = await wallet.signAndSendAllTransactions(transactions.map(t => t.transaction));
        console.log('Transaction signed successfully by wallet');

        console.log(signedTransactions) // TODO: この行を削除

        for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
          const batch = batches[batchIndex];
          const signedTx = transactions[batchIndex];
          const batchRecipients = batch.map((item) => item.recipient);
          const signature = signedTransactions.signature[batchIndex];
