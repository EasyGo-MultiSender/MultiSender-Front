...
export function useTokenTransfer(connection: Connection, publicKey: PublicKey | null) {
  ...
  const logConnectionInfo = useCallback(async () => {
    try {
      ...
      console.log(`Network appears to be: ${getNetworkName(connection.rpcEndpoint)}`);
  ...
  // Mintアドレスの検証関数
  const validateMintAddress = useCallback(async (mintAddress: string): Promise<boolean> => {
    try {
      ...
      if (!mintAccountInfo) {
        console.error(`Mint account not found on current network: ${mintAddress}`);
        throw new Error(`The token (${mintAddress.slice(0, 6)}...${mintAddress.slice(-4)}) was not found on the current network (${getNetworkName(connection.rpcEndpoint)}). Please check your network selection.`);
      }
      ...
      if (!isValidOwner) {
        console.error(`Invalid mint account owner: ${mintAccountInfo.owner.toBase58()}`);
        throw new Error(`The specified address is not a valid token mint on the current network (${getNetworkName(connection.rpcEndpoint)}). Please verify the token mint address.`);
      }
      
      console.log(`Mint account validated: ${mintAddress}, owner: ${mintAccountInfo.owner.toBase58()}`);
      return true;
    } catch (error) {
  ...
  const createBatchTransferTransaction = useCallback(async (
    ...
    if (!mintAddress) {
      ...
      try {
        ...
        console.log(`Token decimals: ${tokenDecimals}, program ID: ${mintInfo.mintAuthority ? mintInfo.mintAuthority.toBase58() : 'None'}`);
        ...
        if (!fromTokenAccountInfo) {
          throw new Error(`You don't have an associated token account for this token (${mintAddress.slice(0, 6)}...${mintAddress.slice(-4)}). Please check if you own this token.`);
        }
  ...
  const getWalletAdapter = useCallback(() => {
    ...
    for (const wallet of possibleWallets) {
      if (wallet && typeof wallet.signTransaction === 'function') {
        console.log(`Using wallet provider: ${wallet.isPhantom ? 'Phantom' : wallet.isSolflare ? 'Solflare' : 'Unknown'}`);
        return wallet;
      }
    }
    ...
    if (walletMultiButton) {
      throw new Error('Please use the Wallet Connect button in the header to connect your wallet');
    } else {
      throw new Error('No compatible wallet found. Please install Phantom or Solflare wallet and refresh the page');
    }
  }, []);
  ...
  const transfer = useCallback(async (params: TransferParams): Promise<TransferResult[]> => {
    ...
    try {
      ...
      console.log(`Starting transfer: ${recipients.length} recipients, amount: ${amount}, mint: ${mint || 'SOL'}`);

      // ネットワーク接続確認
      const connectionOk = await logConnectionInfo();
      if (!connectionOk) {
        throw new Error(`Unable to connect to ${getNetworkName(connection.rpcEndpoint)} network. Please check your internet connection or try a different RPC endpoint.`);
      }
      
      // ウォレットアダプタを事前に取得して検証
      const wallet = getWalletAdapter();
      if (!wallet || typeof wallet.signTransaction !== 'function') {
        throw new Error('Your wallet does not support transaction signing. Please use Phantom or Solflare wallet.');
      }
      ...
      for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
        ...
        try {
          ...
          console.log(`Transaction created for batch ${batchIndex + 1} with ${batch.length} recipients`);
          ...
          try {
            ...
            if (success) {
              ...
              console.log(`Batch ${batchIndex + 1} transaction confirmed successfully: ${signature}`);
            } else {
              // 失敗した場合、エラーとして記録
              results.push({
                signature,
              ...
              console.log(`Batch ${batchIndex + 1} transaction failed confirmation: ${signature}`);
            }
          } catch (signError: any) {
            // ユーザーによる拒否を検出
            if (
              ...
              console.log("User cancelled transaction, stopping further processing");
              break;
            } else {
              console.error("Error during transaction signing:", signError);
              throw new Error(`Failed to sign transaction: ${signError.message || 'Unknown wallet error'}`);
            }
          }
        } catch (error) {
          ...
          if (error instanceof Error) {
            ...
            if (error.message.includes('TokenInvalidAccountOwner')) {
              ...
              errorMessage = 'Invalid token mint address. Please verify the token address.';
            } else if (error.message.includes('TokenAccountNotFound')) {
              errorMessage = "Token account not found. The recipient may not have an account for this token yet.";
      ...
      // メイン処理のエラーを処理
      let errorMessage = 'Transfer operation failed due to an unexpected error';
      if (error instanceof Error) {
        errorMessage = error.message;
      }
  ...
  }, [connection, publicKey, createBatchTransferTransaction, getWalletAdapter, logConnectionInfo]);

  // Sender.tsxからの呼び出し用に、個別の金額を持つ受取人のバッチ処理を提供
  const transferWithIndividualAmounts = useCallback(async (
    ...
    try {
      console.log(`Starting transfer with individual amounts: ${recipientsWithAmounts.length} recipients, mint: ${mint || 'SOL'}`);
      ...
      for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
        ...
        try {
          ...
          console.log(`Transaction created with ${transaction.message.compiledInstructions.length} instructions`);
          console.log("Requesting wallet signature...");
          ...
          try {
            ...
            if (success) {
              ...
              console.log(`Batch ${batchIndex + 1} confirmed successfully with ${batchRecipients.length} recipients`);
            } else {
              results.push({
                signature,
                status: 'error',
          ...
          if (error instanceof Error) {
            ...
            if (error.message.includes('TokenInvalidAccountOwner')) {
              ...
              errorMessage = 'Transaction size exceeded limit. Try sending fewer recipients per batch.';
            }
          }
      ...
      console.error('Transfer operation failed:', error);
      throw error instanceof Error ? error : new Error('Unknown error in transfer operation');
    } finally {
      setLoading(false);
    }
  }, [connection, publicKey, createBatchTransferTransaction, getWalletAdapter, logConnectionInfo]);
